...
// Search a particular faculty by using MOD as a hash function for linear
...
// Hash table class using MOD + Linear Probing
class HashTable {
private:
    Faculty* table;
    bool* occupied;  // To track used slots
    int size;
...
public:
    ...
    void insertFaculty(int id, string name, string dept, string phone) {
        ...
        while (occupied[index]) {
            index = (index + 1) % size;
            if (index == startIndex) {
                cout << "Hash Table is full! Cannot insert record for ID " << id << endl;
                return;
            }
        }
        ...
        cout << "Inserted record for Faculty ID " << id << " at index " << index << endl;
    }
    ...
    void searchFaculty(int id) {
        ...
        cout << "\nFaculty with ID " << id << " not found!" << endl;
    }

    // Display all faculty records
    void displayTable() {
        cout << "\nHash Table (Faculty Database):\n";
        for (int i = 0; i < size; i++) {
            cout << "Index " << i << ": ";
            if (occupied[i]) {
                cout << "[ID: " << table[i].id
...
int main() {
    ...
    do {
        cout << "\n--- Faculty Database (Linear Probing with MOD Hash) ---";
